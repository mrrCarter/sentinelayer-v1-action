...
class PromptLoader:
    ...
    def _resolve_prompt_path(self, file_name: object) -> Optional[Path]:
    ...
    def _select_prompt_entry(
        self, name: Optional[str], scan_mode: str
    ) -> Optional[dict]:
        prompts = self.manifest.get("prompts") or {}
        if not isinstance(prompts, dict) or not prompts:
            return None

        def compatible(entry: object) -> bool:
            if not isinstance(entry, dict):
                return False
            modes = entry.get("scan_modes") or []
            return isinstance(modes, list) and (not modes or scan_mode in modes)
    ...
    def get_prompt(self, name: Optional[str] = None, scan_mode: str = "pr-diff") -> str:
        ...
        prompt_path = self._resolve_prompt_path(entry.get("file"))

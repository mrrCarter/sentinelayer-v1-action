...
class ResponseParser:
    ...
    def parse(self, response_text: str) -> ParseResult:
        ...
        if len(raw) > MAX_RESPONSE_CHARS:
            return ParseResult(
                findings=[],
                parse_errors=["Response exceeded maximum size"],
                raw_response=raw[:MAX_RESPONSE_CHARS],
                no_findings_reported=False,
            )
        ...
        if isinstance(parsed, list):
            for idx, item in enumerate(parsed[:MAX_FINDINGS]):
            ...
            if len(parsed) > MAX_FINDINGS:
                errors.append(
                    f"Response exceeded maximum finding count ({MAX_FINDINGS})"
                )
            return ParseResult(findings, errors, raw, no_findings)
    ...
    def _parse_jsonl(self, content: str) -> tuple[List[dict], List[str]]:
        ...
        for i, line in enumerate(content.split("\n")):
            ...
            try:
                obj = json.loads(line)
                if isinstance(obj, dict) and self._validate_finding(obj):
                    findings.append(obj)
                elif isinstance(obj, dict) and obj.get("no_findings"):
                    continue
                else:
    ...
    def _validate_finding(self, obj: dict) -> bool:
        ...
        for field in ("category", "message"):
            ...
            if (
                ...
                return False
        for field in ("recommendation", "fix_plan"):
            value = obj.get(field, "")
            if not isinstance(value, str) or len(value) > MAX_TEXT_CHARS:
                return False

        file_path = obj.get("file_path")
        if not self._valid_file_path(file_path):
            return False
        ...
        if (
            ...
            or not 1 <= line_start <= MAX_LINE_NUMBER
        ):
            return False
        ...
        if (
            ...
            or not line_start <= line_end <= MAX_LINE_NUMBER
        ):
            return False

...
class _SqlDescendantIndex:
    ...
    def copy(self) -> _SqlDescendantIndex:
        self.shared = True
        return _SqlDescendantIndex(root=self.root, shared=True)
...
def python_interpolated_sql_lines(content: str) -> set[int]:
    ...
    try:
        ...
        # Valid adversarial ASTs must degrade to the bounded token recovery
        # path instead of crashing the deterministic scan.
        return _python_interpolated_sql_lines_from_source(content)
    ...
    for node in ast.walk(tree):
        ...
        if (
            ...
            and (
                ...
                or _has_sql_context(node, parents, context_cache)
            )
        ):
            lines.add(int(getattr(node, "lineno", 1) or 1))
    return lines
...
def _call_argument_expressions_in_order(call: ast.Call) -> list[ast.expr]:
    """Return positional, starred, and keyword values in lexical order."""
...
def _process_sql_expression(
    ...
    while events:
        ...
        if _enqueue_special_sql_expression(
            ...
            continue
        _enqueue_generic_sql_expression(event.node, event.state, events, parents)
...
def _apply_sql_expression_event(
    ...
    if event.kind == "bind":
        ...
        _enqueue_sql_ifexp_branches(current, state, events)
    else:
...
def _enqueue_sql_ifexp_branches(
    ...
    if truth is not None:
        ...
        events.append(_SqlExpressionEvent("visit", branch, state))
        return
    body_state = state.copy()
    else_state = state.copy()
    events.append(_SqlExpressionEvent("merge", node, state, (body_state, else_state)))
    events.append(_SqlExpressionEvent("visit", node.orelse, else_state))
    events.append(_SqlExpressionEvent("visit", node.body, body_state))
...
def _enqueue_special_sql_expression(
    ...
    if isinstance(node, ast.NamedExpr):
        ...
        events.append(_SqlExpressionEvent("visit", node.value, state))
    elif isinstance(node, ast.IfExp):
        events.append(_SqlExpressionEvent("ifexp", node, state))
        events.append(_SqlExpressionEvent("visit", node.test, state))
        ...
        _enqueue_sql_call(node, state, events)
    elif isinstance(node, (ast.Dict, ast.Set)):
        _enqueue_sql_container(node, state, events)
    else:
        return False
    return True
...
def _enqueue_sql_call(
    ...
    sql_arguments = (
        {id(argument) for argument in _sql_execution_value_arguments(node)}
        if _is_sql_execution_call(node)
        else set()
    )
    ordered = [_SqlExpressionEvent("visit", node.func, state)]
    for argument in _call_argument_expressions_in_order(node):
        ordered.append(_SqlExpressionEvent("visit", argument, state))
        if id(argument) in sql_arguments:
            ordered.append(_SqlExpressionEvent("sink", argument, state))
...
def _enqueue_sql_container(
    ...
    if isinstance(node, ast.Dict):
        for key, value in zip(node.keys, node.values, strict=True):
            if key is not None:
                ordered.append(_SqlExpressionEvent("visit", key, state))
            ordered.append(_SqlExpressionEvent("visit", value, state))
            ordered.append(_SqlExpressionEvent("raise", node, state))
    else:
        for element in node.elts:
            ordered.append(_SqlExpressionEvent("visit", element, state))
            ordered.append(_SqlExpressionEvent("raise", node, state))
    events.extend(reversed(ordered))
...
def _enqueue_generic_sql_expression(
    ...
    events.extend(
        _SqlExpressionEvent("visit", child, state)
        ...
        if source:
            ...
            try:
                ...
                try:
                    ...
                    next_state = _analyze_sql_statement_block(
                    ...
                    parsed = True
                    states_by_scope[statement.scope_key] = next_state
        if not parsed:

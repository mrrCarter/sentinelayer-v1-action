...
def resolve_repo_file(repo_root: Path, relative_path: object) -> Path | None:
    """Resolve a scanner input while keeping all reads inside the repository."""
...
class PatternScanner:
    ...
    def _file_matches_pattern(self, file_path: str, pattern: Dict[str, Any]) -> bool:
        ...
        if include_patterns and not any(
            fnmatch.fnmatch(normalized, pat) for pat in include_patterns
        ):
            return False

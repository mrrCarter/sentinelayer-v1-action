...
class ConfigScanner:
    def __init__(self, patterns_dir: Path):
        ...
        self._cicd_patterns = [
            pattern
            for pattern in self._pattern_scanner.patterns
            if str(pattern.get("id", "")).startswith("CICD-")
        ]
    ...
    def scan_file(
        ...
        if (
            ...
            findings.extend(
                self._pattern_scanner.scan_file_with_patterns(
                    Path(rel_path), content, self._cicd_patterns
                )
            )
    ...
    def scan_files(self, files: List[dict], repo_root: Path) -> List[Finding]:
        ...
        for entry in sorted(files, key=lambda item: item.get("path") or ""):
            ...
            if not rel_path:
                continue
            full_path = resolve_repo_file(repo_root, rel_path)
            ...
            findings.extend(
                self.scan_file(Path(rel_path), content, repo_root=repo_root)
            )
        return findings
    ...
    def _tsconfig_refs_are_strict(
        self, config_data: dict, file_path: str, repo_root: Path
    ) -> bool:
        refs = config_data.get("references")
        if not isinstance(refs, list) or not refs:
            return False

        config_path = resolve_repo_file(repo_root, file_path)
        ...
        for entry in refs:
            ...
            strict_value = (
                compiler.get("strict") if isinstance(compiler, dict) else None
            )
    ...
    def _scan_tsconfig(
        ...
        if (
            ...
            return []
        snippet = (
            '"strict": false' if strict_value is False else "strict mode not enabled"
        )
        snippet = _truncate_snippet(snippet)
        return [
            Finding(

...
class EngQualityScanner:
    ...
    def _has_frontend(self) -> bool:
        ...
        return any(
            any(marker in t for marker in frontend_markers) for t in self.tech_stack
        )
    ...
    def _has_backend(self) -> bool:
        ...
        return any(
            any(marker in t for marker in backend_markers) for t in self.tech_stack
        )
    ...
    def _scan_state_updates_in_loops(self, files: dict[str, str]) -> list[Finding]:
        ...
        for path, content in self._iter_files(files, exts=_JS_EXTS):
            ...
            for m in regex.finditer(content):
                ...
                findings.append(
                    self._make_finding(
                        rule, file_path=path, line_start=line, snippet=snippet
                    )
                )
        return findings
    ...
    def _scan_useeffect_without_cleanup(self, files: dict[str, str]) -> list[Finding]:
        ...
        trigger_re = re.compile(
            r"\b(addEventListener|setInterval|subscribe)\b", re.IGNORECASE
        )

        for path, content in self._iter_files(
            files, exts=(".jsx", ".tsx", ".ts", ".js")
        ):
            if ".d.ts" in path.lower():
                continue
            ...
            for idx, line in enumerate(lines):
                ...
                if cleanup_re.search(block):
                    continue
                line_no = idx + 1
                snippet = self._line_snippet(
                    content, line_no, min(line_no + 8, line_no + 8)
                )
                findings.append(
                    self._make_finding(
                        rule,
    ...
    def _scan_useeffect_empty_deps_outer_state(
        ...
        for path, content in self._iter_files(
            ...
            for idx, line in enumerate(lines):
                ...
                if not empty_deps_re.search(block):
                    continue
                if not any(
                    re.search(rf"\b{re.escape(var)}\b", block) for var in state_vars
                ):
                    continue
                line_no = idx + 1
                snippet = self._line_snippet(
                    content, line_no, min(line_no + 10, line_no + 10)
                )
                findings.append(
                    self._make_finding(
                        rule,
    ...
    def _scan_eval_or_function_ctor(self, files: dict[str, str]) -> list[Finding]:
        rule = _Rule(
            ...
            severity="P0",
            category="backend",
            message="Dynamic source execution APIs can enable arbitrary code execution.",
            recommendation="Remove dynamic execution; use safe parsers, validators, and explicit logic.",
            fix_plan="Delete dynamic execution paths and replace them with explicit parsing and dispatch. Example: parse JSON with a strict schema and route known commands via an allowlist map.",
        )
        findings: list[Finding] = []
        for path, content in self._iter_files(files, exts=_BACKEND_EXTS):
            if self._is_test_file(path):
    ...
    def _scan_sql_string_concat(self, files: dict[str, str]) -> list[Finding]:
        ...
        sql_operations = "|".join(("SELECT", "INSERT", "UPDATE", "DELETE"))
        concat_pattern = (
            rf"(['\"]).{{0,120}}\b({sql_operations})\b"
            r".{0,200}\1\s*\+\s*[A-Za-z_][A-Za-z0-9_]*"
        )
        ...
        py_fstring = re.compile(
            rf"f(['\"]).{{0,200}}\b({sql_operations})\b.{{0,400}}\{{[^}}]+\}}.*\1",
            re.IGNORECASE,
        )
    ...
    def _scan_auth_routes_without_rate_limit(
        self, files: dict[str, str]
    ) -> list[Finding]:
        rule = _Rule(
            pattern_id="EQ-011",
            ...
            fix_plan="Insert rate-limiter middleware in the auth route chain before handler logic. For example, place the limiter between route declaration and handler, and enforce per-IP and per-account limits.",
        )
        findings: list[Finding] = []
        route_re = re.compile(
    ...
    def _scan_http_calls_without_timeout(self, files: dict[str, str]) -> list[Finding]:
        ...
        axios_re = re.compile(
            r"\baxios\.(get|post|put|patch|delete)\s*\(", re.IGNORECASE
        )
        fetch_re = re.compile(r"\bfetch\s*\(", re.IGNORECASE)
        httpx_call_re = re.compile(
            r"\bhttpx\.(get|post|put|patch|delete)\s*\(", re.IGNORECASE
        )
        httpx_client_re = re.compile(
            r"\bhttpx\.(AsyncClient|Client)\s*\(", re.IGNORECASE
        )
    ...
    def _scan_unbounded_retry_loops(self, files: dict[str, str]) -> list[Finding]:
        ...
        bounded_re = re.compile(
            r"\b(max_attempts|max_retries|attempts?\s*<)\b", re.IGNORECASE
        )
    ...
    def _scan_rate_limit_fail_open(self, files: dict[str, str]) -> list[Finding]:
        ...
        likely_limiter_file_re = re.compile(
            r"\brate\s*limit|\blimiter\b", re.IGNORECASE
        )
    ...
    def _scan_mutations_without_idempotency(
        self, files: dict[str, str]
    ) -> list[Finding]:
        rule = _Rule(
            pattern_id="EQ-015",
            ...
            fix_plan="Require Idempotency-Key on write endpoints, persist the request hash and response, and replay the prior response for duplicate keys. Reject key reuse when payload hashes differ.",
        )
        findings: list[Finding] = []
    ...
    def _scan_missing_request_id_schema(self, files: dict[str, str]) -> list[Finding]:
        ...
        for path, content in self._iter_files(files, exts=_BACKEND_EXTS):
            ...
            findings.append(
                self._make_finding(
                    rule, file_path=path, line_start=1, snippet="", confidence=0.4
                )
            )
        return findings

    def _scan_external_calls_without_fallback(
        self, files: dict[str, str]
    ) -> list[Finding]:
        rule = _Rule(
            pattern_id="EQ-017",
        ...
        for path, content in self._iter_files(files, exts=_BACKEND_EXTS):
            ...
            findings.append(
                self._make_finding(
                    rule, file_path=path, line_start=1, snippet="", confidence=0.35
                )
            )
        return findings
    ...
    def _scan_dockerfile_user(self, files: dict[str, str]) -> list[Finding]:
        ...
        for path, content in self._iter_files(files):
            if path.lower().endswith("/dockerfile") or path.lower() == "dockerfile":
                ...
                findings.append(
                    self._make_finding(
                        rule, file_path=path, line_start=1, snippet="", confidence=0.9
                    )
                )
        return findings
    ...
    def _scan_oauth_callback_missing_state(
        self, files: dict[str, str]
    ) -> list[Finding]:
        rule = _Rule(
            pattern_id="EQ-023",
    ...
    def _scan_terraform_remote_backend(self, files: dict[str, str]) -> list[Finding]:
        ...
        tf_files = [
            (p, c) for p, c in self._iter_files(files) if p.lower().endswith(".tf")
        ]
        if not tf_files:
            return []
        ...
        return [
            self._make_finding(
                rule, file_path=first_path, line_start=1, snippet="", confidence=0.7
            )
        ]
    ...
    def _scan_env_committed(self, files: dict[str, str]) -> list[Finding]:
        ...
        for path, _content in self._iter_files(files):
            ...
            if (
                ...
                or p.endswith(".env.template")
                or p.endswith(".env.sample")
            ):
                continue
            findings.append(
                self._make_finding(
                    rule, file_path=path, line_start=1, snippet="", confidence=0.95
                )
            )
        return findings
    ...
    def _scan_workflow_hardcoded_secrets(self, files: dict[str, str]) -> list[Finding]:
        ...
        secret_key_re = re.compile(
            r"(?:^|_)(token|secret|password|api[_-]?key)(?:$|_)", re.IGNORECASE
        )

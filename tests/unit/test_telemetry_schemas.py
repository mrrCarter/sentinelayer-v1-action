...
def test_tier2_includes_optional_spec_compliance() -> None:
    collector = TelemetryCollector(run_id="test", repo_full_name="acme/app")
    payload = build_tier2_payload(
        collector=collector,
        repo_owner="acme",
        ...
        findings_summary=[],
        idempotency_key="xyz",
        spec_compliance=SpecComplianceTelemetry(
            spec_hash="c" * 64,
            sections_checked=["5", "7"],
            sections_violated=["5"],
        ),
    )
    assert payload["spec_compliance"] is not None
    assert payload["spec_compliance"]["spec_hash"] == "c" * 64

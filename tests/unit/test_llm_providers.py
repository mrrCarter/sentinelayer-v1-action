...
@pytest.mark.anyio
async def test_anthropic_provider_calls_messages_create(
...
def test_missing_anthropic_sdk_raises_clear_error(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
...
@pytest.mark.anyio
async def test_gemini_provider_calls_generate_content(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
...
def test_cost_estimation_per_provider() -> None:
    ...
    assert (
        abs(openai.estimate_cost("gpt-4o", 10000, 1000) - ((10 * 0.005) + (1 * 0.015)))
        < 1e-6
    )
    ...
    assert (
        abs(
            anthropic.estimate_cost("claude-sonnet-4-5-20250929", 1_000_000, 1_000_000)
            - 18.0
        )
        < 1e-6
    )
    ...
    assert (
        abs(gemini.estimate_cost("gemini-2.5-flash", 1_000_000, 1_000_000) - 0.75)
        < 1e-6
    )

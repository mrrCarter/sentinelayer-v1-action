...
def test_scan_files_aggregates_findings(
    scanner: PatternScanner, tmp_path: Path
) -> None:
    repo_root = tmp_path / "repo"
...
def test_scan_files_refuses_repository_escape(
    scanner: PatternScanner, tmp_path: Path
) -> None:
    repo_root = tmp_path / "repo"
    repo_root.mkdir()
...
def test_scanner_performance_1000_files(
    scanner: PatternScanner, benchmark_fixture
) -> None:
    start = time.time()

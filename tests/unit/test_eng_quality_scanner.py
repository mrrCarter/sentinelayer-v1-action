...
def test_python_sql_concatenation_fails_closed_at_parser_resource_limit() -> None:
    source = (
        'query = "SELECT * FROM users WHERE id = " + '
    ...
    with pytest.raises(DeterministicAnalysisBudgetExceeded) as caught:
        scanner.scan({"src/stress_query.py": source})
...
def test_sql_scan_fails_closed_when_valid_ast_analysis_recurses(monkeypatch) -> None:
    from omargate.analyze.deterministic import eng_quality_helpers
...
def test_python_sql_concatenation_parser_exhaustion_fails_closed() -> None:
    ...
    with pytest.raises(DeterministicAnalysisBudgetExceeded) as caught:
        EngQualityScanner(tech_stack=["Python"]).scan({"src/stress_query.py": source})
...
def test_deep_dotted_binding_index_is_compact_and_fully_metered(
    ...
    assert all(
        larger <= smaller * 5 for smaller, larger in zip(work_units, work_units[1:])
    )
...
def test_python_work_budget_never_returns_partial_findings(
    ...
    source = 'payload = f"SELECT {column}"\ncursor.execute(payload)\n'

    with pytest.raises(DeterministicAnalysisBudgetExceeded) as caught:
        EngQualityScanner(tech_stack=["Python"]).scan({"src/work_budget.py": source})
...
def _nested_conditional_sql_source(depth: int = 8_000) -> str:
    ...
    for index in reversed(range(depth)):
        expression = f'f"SELECT {{v{index}}}" if c{index} else {expression}'
...
def test_python_analysis_work_budget_is_typed_at_helper_boundary() -> None:
    assert (
        eng_quality_helpers.python_interpolated_sql_lines(
            _nested_comprehension_source(100),
    ...
    assert error.budget_kind == "python_analysis_work_units"
    assert error.limit == Limits.MAX_PYTHON_ANALYSIS_WORK_UNITS
...
def test_malformed_httpx_recovery_resource_failure_is_typed_and_atomic(
    ...
    source = 'httpx.get("https://example.test", **{"timeout": 5})\ndef broken(:\n'
    not_returned = object()
    result: object = not_returned

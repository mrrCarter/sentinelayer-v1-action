...
def test_quota_header_detection_ignores_generic_api_rate_limit_headers() -> None:
    assert not _has_quota_headers({"x-ratelimit-limit": "100"})
    assert not _has_quota_headers({"x-ratelimit-reset": str(int(time.time()) + 60)})
    assert _has_quota_headers({"ratelimit-unified-status": "allowed"})
    assert _has_quota_headers({
        "ratelimit-unified-5h-utilization": "0.92",
        "ratelimit-unified-reset": str(int(time.time()) + 7200),
    })
